  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (result.nextStep.signInStep === "CONFIRM_SIGN_IN_WITH_SMS_CODE") {
        setAwaitingOtp(true); // show OTP field
      } else if (result.isSignedIn) {
        await finishLogin();
      }
    } catch {
      // Cognito not configured yet — demo mode
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#08080A", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Syne', sans-serif" }}>
      <motion.div
        ...
        {awaitingOtp ? (
          <>
            <p style={{ color: "#888", fontFamily: "'JetBrains Mono', monospace", fontSize: 12, marginBottom: 16 }}>
              OTP sent to {phone}
            </p>
            ...
            {error && <div style={{ color: "#E8554E", fontFamily: "'JetBrains Mono', monospace", fontSize: 12, marginBottom: 16 }}>{error}</div>}
            <motion.button
              whileTap={{ scale: 0.97 }}
              onClick={handleOtp}
              disabled={loading}
              style={{ width: "100%", padding: "14px", borderRadius: 8, border: "none", background: loading ? "#333" : "#C0272D", color: "#fff", fontSize: 15, fontWeight: 700, cursor: loading ? "not-allowed" : "pointer", fontFamily: "'Syne', sans-serif" }}>
              {loading ? "Verifying..." : "Verify OTP →"}
            </motion.button>
          </>
        ) : (
          /* Phone + password screen — unchanged from your original */
          <>
            {[
